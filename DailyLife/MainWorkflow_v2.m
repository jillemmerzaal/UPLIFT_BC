...
% * averge of percentage of functional use
...
for subj = 1%6:10
    ...
    if check_subj == 7 && size(content_check,1) > 3
        ...
        if ~isempty(double(pyOut.hip{1,1}))
            ...
            hip.data(:,1) = double(py.array.array('d', py.numpy.nditer(pyOut.hip{2})))';
            hip.data(:,2) = double(py.array.array('d', py.numpy.nditer(pyOut.hip{3})))';
            hip.data(:,3) = double(py.array.array('d', py.numpy.nditer(pyOut.hip{4})))';
            hip.data(:,4) = double(py.array.array('d', py.numpy.nditer(pyOut.hip{5})))';
